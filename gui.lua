...
local function Drag(frame)
    ...
    frame.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then dragToggle, dragStart, startPos = true, i.Position, frame.Position end end)
    UserInputService.InputChanged:Connect(function(i) if dragToggle and i.UserInputType == Enum.UserInputType.MouseMovement then
        local delta = i.Position - dragStart
        frame.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
    end end)
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then dragToggle = false end end)
end
Drag(Main)
...
function UI:CreateTab(name)
    ...
    function Elements:Keybind(txt, def, cb)
        ...
        local KeyLabel = Create("TextLabel", { Parent = KFrame, AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = Color3.fromRGB(30, 30, 35), Position = UDim2.new(1, -15, 0.5, 0), Size = UDim2.new(0, 70, 0, 25), Font = Enum.Font.GothamBold, Text = def.Name, TextColor3 = Theme.Accent, TextSize = 12 })
